...
class ANETdetection(object):
    ...
    #         c_s=c_score[i]
    #         for c in np.argsort(c_s[:20],axis=-1)[-2:]:
    ...
    #                 if e[j] - s[j] >= 1:
    #                     # L = e[j]-s[j]
    #                     # Ts = (s[j]-int(L/4)) if (s[j]-int(L/4)) >0 else 0
    #                     # Te = (e[j]+int(L/4)) if (e[j]+int(L/4)) < len(vid_pred_diff)  else len(vid_pred_diff)
    #                     # iscore = np.sum(tmp[s[j] : e[j]])/L-(np.sum(tmp[Ts:Te])-np.sum(tmp[s[j] : e[j]]))/(Te-Ts-L)
    #                     seg = tmp[s[j]: e[j]]
    #                     segment_predict.append(
    #                         # [i, s[j], e[j], iscore,
    #                         [i, s[j], e[j], np.max(seg) + 0.7 * c_s[c],  # np.max(tmp[s[j] : e[j]])+0.7*c_s[c],
    #                          c]
    #                     )
    ...
    # def _import_prediction(self,predictions):
    #     instance_logits,attn=predictions
    #     pred=[]
    ...
    #
    #     # generate proposal based on Attention
    #     for i in range(len(attn)):
    #         c_s=c_score[i]
    #         for c in np.argsort(c_s,axis=-1)[-2:]:
    #             # if 1/(1+np.exp(-c_s[c]))<0.1:
    #             #     continue
    #             if c_s[c]<0.1:
    #                 continue
    #             att=attn[i]
    #             vid_pred = np.concatenate(
    #                 [np.zeros(1), (att > threshold).astype("float32"), np.zeros(1)], axis=0
    #             )
    #
    #             vid_pred_diff = [
    #                 vid_pred[idt] - vid_pred[idt - 1] for idt in range(1, len(vid_pred))
    ...
    #             for j in range(len(s)):
    #                 if e[j] - s[j] >= 1:
    #                     mean_logit=np.mean(instance_logits[i][s[j]:e[j]],axis=0)
    #                     max_logit=np.max(instance_logits[i][s[j]:e[j]],axis=0)[c]
    #                     softmaxed_proposal_score=np.exp(mean_logit)/np.sum(np.exp(mean_logit))
    #
    #                     # if softmaxed_proposal_score[c]<0.1:
    #                     #     continue
    #
    #                     # L = e[j]-s[j]
    #                     # Ts = (s[j]-int(L/4)) if (s[j]-int(L/4)) >0 else 0
    ...
    #                     segment_predict.append(
    #                         # [i, s[j], e[j], iscore,
    #                         [i, s[j], e[j],max_logit + 0.7 * c_s[c],  # np.max(tmp[s[j] : e[j]])+0.7*c_s[c],
    #                          c]
    #                     )
    #
    #     segment_predict = np.array(segment_predict)
    ...
    def _import_prediction(self, predictions):
        ...
        # to rewrite to reduce unexpected false alarm items
        for i in range(len(predictions)):
            c_s=c_score[i]
            # avg_c_s=np.mean(c_s,axis=-1,keepdims=False)
            # for c in self.templabelidx:
            softmaxed_c_s=np.exp(c_s)/np.sum(np.exp(c_s))

            # for c in np.argsort(c_s,axis=-1)[-3:]:
            for c in range(c_s.shape[0]):
                ...
                #     continue
                # else:
                # tmp = gaussian_smooth(predictions[i][:,c],30)
                if self.args.dataset_name=='Thumos14reduced':
                    tmp=predictions[i][:,c]
                else:
                    tmp=smooth(predictions[i][:,c])
                ...
                # threshold=np.max(tmp) - (np.max(tmp) - np.min(tmp))*0.7
                # threshold=np.sum(tmp)/(tmp.shape[0])
                # threshold = np.max(tmp) - (np.max(tmp) - np.min(tmp)) * thres
                # threshold=tmp[np.argsort(tmp)[int(tmp.shape[0]*thres)]]
                vid_pred = np.concatenate(
                    [np.zeros(1), (tmp > threshold).astype("float32"), np.zeros(1)], axis=0
                )
                vid_pred_diff = [
