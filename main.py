...
if __name__ == '__main__':
    ...
    model = getattr(model, args.use_model)(dataset, device, dataset.num_class, opt=args).to(device)
    ...
    for itr in tqdm(range(args.max_iter)):
        ...
        if itr % args.interval == 0 and not itr == 0:
            ...
            total_loss = 0
            torch.save(model.state_dict(), './ckpt/last_re_rank_' + args.model_name + '.pkl')
            ...
            if cond:
                torch.save(model.state_dict(), './ckpt/best_re_rank_' + args.model_name + '.pkl')
                max_map = dmap
