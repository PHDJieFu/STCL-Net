...
def test(itr, dataset, args, model, logger, device, pool):
    ...
    while not done:
        ...
        with torch.no_grad():
            outputs = model(Variable(features), device, is_training=False, seq_len=seq_len)

            # modify the cas with intra_cas and inter_cas
            # outputs['cas'] = 0.5 * outputs['cas'] + 0.5 * (0.7 * outputs['intra_cas'] + 0.3 * outputs['inter_cas'])             ###############
            # outputs['cas'] = outputs['inter_cas']

            # re_rank_cas = re_rank_clser(outputs['feat'])                       # (B, T, D) -> (B, T, C+1)     new action score
            # outputs['cas'] = outputs['cas'] + re_rank_cas
        ...
        tmp = F.softmax(torch.mean(torch.topk(logits, k=int(np.ceil(len(features)/8)), dim=0)[0], dim=0), dim=0).cpu().data.numpy()         # 错???
    ...
    if 'Thumos14' in args.dataset_name:
        ...
        dmap = dmap_detect.evaluate()                               # 这里是根据proposal和标注的区间计算detection mean average precision
    else:
    ...
    # 这里使用的是reduced之后的测试集,并不是原始的‘Thumos14’,所以下面这个判断条件不满足
