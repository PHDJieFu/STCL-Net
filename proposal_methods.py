...
def _get_vid_score(pred):
    ...
    if args is None:
        ...
        topk_mean = get_topk_mean(pred, k)
        # ind = topk_mean > -50
...
def multiple_threshold_hamnet(vid_name, data_dict):
    ...
    cas_pred = cas_supp[0].cpu().numpy()[:, pred]               # (T, act_num)
    num_segments = cas_pred.shape[0]
    cas_pred = np.reshape(cas_pred, (num_segments, -1, 1))      # (T, act_num, 1)
    ...
    for i in range(len(act_thresh)):
        ...
        proposals = utils.get_proposal_oic_2(seg_list,
                                            ...
                                            args.feature_fps,
                                            num_segments,
                                            gamma=args.gamma_oic)           # [class, score, start, end]
